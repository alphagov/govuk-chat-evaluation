...
```bash
./bin/export_aws_credentials.sh
```

The script writes `.env.aws.local`, which is automatically loaded by the CLI on startup. Re-run it whenever the credentials expire.
