from typing import List, Type, TypeVar, cast, Optional, Union
...
class AbsenceOfFactualContradictions(BaseMetric):
    ...
    async def a_measure(
        ...
        **kwargs,
        ...
        with metric_progress_indicator(
            ...
            self.truths = await self._generate_truths(
                cast(str, test_case.expected_output)
            )
            self.claims = await self._generate_claims(
                cast(str, test_case.actual_output)
            )
            self.verdicts = await self._generate_verdicts(self.claims, self.truths)
            ...
            self.verbose_logs = construct_verbose_logs(
                self,
                steps=[
                    f"Truths:\n{prettify_list(self.truths.truths)}",
    ...
    async def _generate_verdicts(
        ...
        prompt = self.evaluation_template.generate_verdicts(
            claims=claims.claims, ground_truth=ground_truth_str
        )
    ...
    async def _generate_reason(
        ...
        if self.using_native_model:
            res, cost = cast(
                tuple[str, float],
                await self.model.a_generate(prompt),
            )
            self.evaluation_cost = (self.evaluation_cost or 0.0) + cost
            return res
        else:
            return cast(str, await self.model.a_generate(prompt))
